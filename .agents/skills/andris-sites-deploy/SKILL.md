...
2. Check that project data and migrations are present and current when the release includes schema or CMS changes. Use the Supabase skill for database operations; never assume a local `.env` proves that production is configured.
